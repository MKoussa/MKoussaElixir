defmodule Mkoussaelixir.ShoppingCartTest do
  use Mkoussaelixir.DataCase

  alias Mkoussaelixir.ShoppingCart

  describe "carts" do
    alias Mkoussaelixir.ShoppingCart.Cart

    import Mkoussaelixir.ShoppingCartFixtures
    ...
    test "create_cart/1 with valid data creates a cart" do
      valid_attrs = %{user_uuid: "some user_uuid"}

      assert {:ok, %Cart{} = cart} = ShoppingCart.create_cart(valid_attrs)
      assert cart.user_uuid == "some user_uuid"
    end
    ...
    test "update_cart/2 with valid data updates the cart" do
      cart = cart_fixture()
      update_attrs = %{user_uuid: "some updated user_uuid"}

      assert {:ok, %Cart{} = cart} = ShoppingCart.update_cart(cart, update_attrs)
      assert cart.user_uuid == "some updated user_uuid"
    end
  ...
  describe "cart_item" do
    alias Mkoussaelixir.ShoppingCart.CartItem

    import Mkoussaelixir.ShoppingCartFixtures

    @invalid_attrs %{price_when_carted: nil, quantity: nil}

    test "list_cart_item/0 returns all cart_item" do
      cart_item = cart_item_fixture()
      assert ShoppingCart.list_cart_item() == [cart_item]
    end
